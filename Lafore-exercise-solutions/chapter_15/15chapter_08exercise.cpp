...
int main()
{
	...
	// – first1 и last1 определяют копируемую последовательность,
	...
	// проверим полученные индексы на ошибки
	...
	if (first2 < 0 || first2 >= MAX)
	{
		wcout << L"Ошибка! first2 может быть только в диапазоне 0.." << MAX - 1 << endl;
		exit(1);
	}
	if (first1 > last1)
	{
