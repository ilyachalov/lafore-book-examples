...
elevator::elevator(building* ptrB) : ptrBuilding(ptrB)
{
	...
	for (int j = 0; j < NUM_FLOORS; j++) // пассажир еще не
		destination[j] = false;          //     нажимал кнопку с этажом назначения
	loading_timer = 0;                   // еще не началась посадка пассажира
	unloading_timer = 0;                 // еще не началась высадка пассажира
}
...
void elevator::car_tick1() // тик 1 для лифта (выбор действия)
{
	...
	dests_display();       // отображение метки этажа, куда едет пассажир
	if (loading_timer)     // отсчет времени посадки пассажира
		--loading_timer;
	if (unloading_timer)   // отсчет времени высадки пассажира
		--unloading_timer;
...
void elevator::car_display() // отображение лифта на экране
{
	...
	case 3:
		...
		break;                            // пассажир хочет войти (слева)
	case 2:
		wcout << L" \x2588\x263A\x2588 "; // пассажир внутри кабины с открытой дверью,
		get_destinations();               // получаем этаж назначения
		break;
	case 1:
		wcout << L" \x2588\x2588\x2588 "; // дверь лифта закрывается,
		break;                            // пассажир внутри кабины и его не видно
	case 0:
		wcout << L" \x2588\x2588\x2588 "; // дверь лифта закрыта,
		break;                            // пассажира нет (по умолчанию)
	}
	...
	case 3:
		...
		break;                            // виден пассажир внутри кабины
	case 2:
		wcout << L"\x2588 \x2588\x263A";  // дверь лифта открыта,
		break;                            // пассажир вышел из лифта (направо)
	case 1:
		wcout << L"\x2588\x2588\x2588 ";  // дверь лифта закрылась,
		break;                            // пассажира не видно
	case 0:
		wcout << L"\x2588\x2588\x2588 ";  // дверь лифта закрыта,
		break;                            // пассажира нет (по умолчанию)
	}
...
void elevator::dests_display() const // отображение метки этажа, куда едет пассажир
{
	for (int j = 0; j < NUM_FLOORS; j++)
	{
...
void elevator::decide() // принятие решения о следующем действии
{
	...
	// эти флаги указывают, есть ли запросы с этажей (ЗЭ) или конечный пункт выше и ниже нас
	...
	// если текущий этаж является пунктом назначения, высадить пассажира
	if (destination[current_floor] == true)
	{
		destination[current_floor] = false; // удалить этот пункт назначения из списка
		if (!unloading_timer)               // начать высадку пассажира
	...
	{
		if (destination[j])                      // если он является пунктом назначения,
	...
	// если имеется пункт назначения, а мы стоим или движемся к нему,
	...
	if ((ptrBuilding->get_floor_req(UP, current_floor) && current_dir != DN))
	{
		...
		if (!loading_timer)                 // начать посадку пассажира
...
void elevator::get_destinations() // получение номера конечного этажа (лифт остановился)
{
	...
	set_cursor_pos(1, 23);
	wcout << L"Введите этаж назначения: ";
